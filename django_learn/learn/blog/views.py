...
def get_files(root_path,cur_path, allow_types=[]):
	...
	for item in items:
		...
		if os.path.isdir(item_fullname):
			...
			if is_allow_list:
				files.append({
					"url":urllib.basejoin(settings.gSettings.MEDIA_URL ,os.path.join(os.path.relpath(cur_path,root_path),item).replace("\\","/" )),
...
def UploadFile(request):
	...
	UploadFieldName=request.GET.get(upload_field_name[action],settings.UEditorUploadSettings.get(action,"upfile"))
	...
	if upload_allow_type.has_key(action):
		allow_type= list(request.GET.get(upload_allow_type[action],settings.UEditorUploadSettings.get(upload_allow_type[action],"")))
	...
	max_size=long(request.GET.get(upload_max_size[action],settings.UEditorUploadSettings.get(upload_max_size[action],0)))
	if  max_size!=0:
	...
	if state=="SUCCESS":
		if action=="uploadscrawl":
			...
			upload_module_name = settings.UEditorUploadSettings.get("upload_module", None)
	...
	#返回数据
	return_info = {
		'url': urllib.basejoin(settings.gSettings.MEDIA_URL , OutputPathFormat) ,				# 保存后的文件名称
...
def catcher_remote_image(request):
	...
	allow_type= list(request.GET.get("catcherAllowFiles",settings.UEditorUploadSettings.get("catcherAllowFiles","")))
	max_size=long(request.GET.get("catcherMaxSize",settings.UEditorUploadSettings.get("catcherMaxSize",0)))
	...
	for remote_url in remote_urls:
		...
		if remote_original_ext  in allow_type:
			...
			catcher_infos.append({
				"state":state,
				"url":urllib.basejoin(settings.gSettings.MEDIA_URL , o_path_format),
...
def get_output_path(request,path_format,path_format_var):
	#取得输出文件的路径
	OutputPathFormat=(request.GET.get(path_format,settings.UEditorSettings["defaultPathFormat"]) % path_format_var).replace("\\","/")
	#分解OutputPathFormat
	OutputPath,OutputFile=os.path.split(OutputPathFormat)
	OutputPath=os.path.join(settings.gSettings.OUTPUT_DIR,OutputPath)
	print 'OutputPath:', OutputPath
	if not OutputFile:#如果OutputFile为空说明传入的OutputPathFormat没有包含文件名，因此需要用默认的文件名
		OutputFile=settings.UEditorSettings["defaultPathFormat"] % path_format_var
...
def save_scrawl_file(request,filename):
	import base64
	try:
		content=request.POST.get(settings.UEditorUploadSettings.get("scrawlFieldName","upfile"))
		f = open(filename, 'wb')
