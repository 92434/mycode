...
public class NameCollector {
	...
	public NameCollector() {
		try {
			listmgr = Runtime.getRuntime().exec(
					".\\interNet\\listmgr\\debug\\listmgr.exe");
		...
		try {
			...
			while (true) {
				...
				if (byteCount != -1) {
					...
					DatagramPacket echo = new DatagramPacket(echoBuf,
							echoBuf.length, senderAddress, senderPort);
					socket.send(echo);
				} else
