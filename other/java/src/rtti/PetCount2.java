...
public class PetCount2 {
	public static void main(String[] args) {
		...
		for (int i = 0; i < pets.size(); i++) {
			...
			if (o instanceof Pet)
				((Counter) h.get("class rtti.Pet")).i++;
			if (o instanceof Dog)
				((Counter) h.get("class rtti.Dog")).i++;
			if (o instanceof Pug)
				((Counter) h.get("class rtti.Pug")).i++;
			if (o instanceof Cat)
				((Counter) h.get("class rtti.Cat")).i++;
			if (o instanceof Rodent)
				((Counter) h.get("class rtti.Rodent")).i++;
			if (o instanceof Gerbil)
				((Counter) h.get("class rtti.Gerbil")).i++;
			if (o instanceof Hamster)
				((Counter) h.get("class rtti.Hamster")).i++;
		}
		for (int i = 0; i < pets.size(); i++)
