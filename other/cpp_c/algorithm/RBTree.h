...
public:
    ...
    */
    ...
    bool Insert(KEY key,U data)
    {
        ...
        RB_Node* index = m_root;
        while(index!=m_nullNode)//根结点为空，跳出来
        {
            insert_point = index;
            if(key<index->key)
            {
        ...
        if(insert_point==m_nullNode) //如果插入的是一颗空树，根结点定位到插入结点，根结点爸爸定位到空，空结点左右子，空结点爸爸定位到根结点，
        {
            m_root = insert_node;
        ...
        {
            if(key<insert_point->key)//键小于插入结点，左子定位到插入结点
            {
                insert_point->left = insert_node;
            }
            else//键大于等于插入结点，右子定位到插入结点
            {
    ...
    */
    ...
    {
        while(node->parent->RB_COLOR==RED)//爸爸为红
        {
            if(node->parent==node->parent->parent->left)   //爸爸为爷爷的红左子，找爷爷右子，如果爷爷两孩子为红，两孩子颜色置红，爷爷置黑，结点定位到爷爷。
            {
                ...
                else if(uncle->RB_COLOR == BLACK )  //插入情况2：z的叔叔y是黑色的，。//爸爸为红左子，爸爸兄弟为黑右子
                {
                    if(node == node->parent->right) //本身是右孩子，结点定位到爸爸，以定位后的结点左旋转
                    {
                        node = node->parent;
                        RotateLeft(node);
                    }
                    else                 //插入情况3：z的叔叔y是黑色的，爸爸为左红子，爸爸兄弟为右黑子，本身是左孩子，爸爸，爸爸换颜色，以爸爸为轴右旋转。
                    {
                        node->parent->RB_COLOR = BLACK;
                        node->parent->parent->RB_COLOR = RED;
            ...
            else //这部分是针对为插入情况1中，z的父亲现在作为祖父的右孩子了的情况，而写的。//爸爸为右红子
                //15 else (same as then clause with "right" and "left" exchanged)
            {
                RB_Node* uncle = node->parent->parent->left;//找爸爸左兄弟
                if(uncle->RB_COLOR == RED)//爸爸左兄弟为红，即爸爸，爸爸兄弟为红，爸爸，爸爸兄弟置黑，爷爷置红，结点定位到爷爷。
                {
                    node->parent->RB_COLOR = BLACK;
                    uncle->RB_COLOR = BLACK;
                ...
                else if(uncle->RB_COLOR == BLACK)//爸爸左兄弟为黑，
                {
                    if(node == node->parent->left)//本身是左子，结点定位到爸爸，以定位后的结点为轴右旋转。
                    {
                        node = node->parent;
                        RotateRight(node);     //与上述代码相比，左旋改为右旋
                    }
                    else//本身为右子，爸爸，爷爷颜色互换，以爷爷为轴左旋转。
                    {
                        node->parent->RB_COLOR = BLACK;
                        node->parent->parent->RB_COLOR = RED;
    ...
    */
    ...
    {
        while(node!=m_root && node->RB_COLOR==BLACK)//结点颜色为红或结点为根结点，跳出。
        {//结点为黑且不为根
            if(node == node->parent->left)//本身是左
            {
                RB_Node* brother = node->parent->right;//找到右兄弟
                if(brother->RB_COLOR==RED)   //情况1：x的兄弟w是红色的。
                {
                    brother->RB_COLOR = BLACK;//兄弟爸爸颜色互换，以爸爸为轴左旋转。
                    node->parent->RB_COLOR = RED;
                    RotateLeft(node->parent);
                }
                else     //情况2：x的兄弟w是黑色的，
                {
                    if(brother->left->RB_COLOR == BLACK && brother->right->RB_COLOR == BLACK)//右兄弟两孩子都为黑
                        //且w的俩个孩子都是黑色的。
                    {
                        brother->RB_COLOR = RED;//置兄弟颜色为红，重新定位结点到爸爸
                        node = node->parent;
                    }
                    else if(brother->right->RB_COLOR == BLACK)//只有右孩子为黑,左孩子为红
                        //情况3：x的兄弟w是黑色的，w的右孩子是黑色（w的左孩子是红色）。
                    {
                        brother->RB_COLOR = RED;//兄弟，兄弟左孩子颜色互换，以兄弟为轴右旋转
                        brother->left->RB_COLOR = BLACK;
                        RotateRight(brother);
                    }
                    else if(brother->right->RB_COLOR == RED)//兄弟右孩子为红（左孩子颜色不管），置兄弟颜色为爸爸颜色，爸爸，兄弟右孩子置黑，以爸爸为轴左旋转。重新定位结点为根结点
            ...
            {//本身为右
                RB_Node* brother = node->parent->left;//找到左兄弟
                if(brother->RB_COLOR == RED)//左兄弟为红，兄弟爸爸颜色互换，以爸爸为轴右旋转
                {
                ...
                {
                    if(brother->left->RB_COLOR==BLACK && brother->right->RB_COLOR == BLACK)//左兄弟两孩子为黑
                    {
                        brother->RB_COLOR = RED;
                        node = node->parent;
                    }
                    else if(brother->left->RB_COLOR==BLACK)//左兄弟左孩子为黑，右孩子为红，兄弟右孩子颜色互换，以兄弟为轴右旋转
                    {
                        brother->RB_COLOR = RED;
                        brother->right->RB_COLOR = BLACK;
                        RotateLeft(brother);
                    }
                    else if(brother->left->RB_COLOR==RED)//左兄弟左孩子为红，右孩子不管，置兄弟颜色为爸爸颜色，爸爸，兄弟左孩子颜色置黑，以爸爸为轴右旋转，重新定位结点为根结点
                    {
                        brother->RB_COLOR = node->parent->RB_COLOR;
                        node->parent->RB_COLOR = BLACK;
                        brother->left->RB_COLOR = BLACK;
    ...
    //
    inline RB_Node* InOrderPredecessor(RB_Node* node)//找前继
    {
    ...
    inline RB_Node* InOrderSuccessor(RB_Node* node)
    {
        ...
        if(result == m_nullNode)//没有右子
        {
            RB_Node* index = node->parent;
            result = node;
            while(index!=m_nullNode && result == index->right)//有爸爸，且本身是右子，继续找爸爸。否则（没爸爸，或不是右子）返回爸爸
            {
                result = index;
                index = index->parent;
            }
        ...
        return result;//一定要返回比它大的第一个值。否则返回空。
    }
...
private:
    ...
    {
#define color(rbcolor) (rbcolor == RED) ? ("r") : ("b")

        if(node == NULL || node == m_nullNode) return;

        if(node->parent == NULL || node->parent == m_nullNode)//根结点
        {
            printf("root:%d\n", node->data);
        }
        else if(node->parent->left == node)
        {
            printf("left(%s):%d,\tparent(%s):%d\n", color(node->RB_COLOR), node->data, color(node->parent->RB_COLOR), node->parent->data);
        }
        else if(node->parent->right == node)
        {
            printf("right(%s):%d,\tparent(%s):%d\n", color(node->RB_COLOR), node->data, color(node->parent->RB_COLOR), node->parent->data);
        }
