...
static void gf_enable_irq(struct gf_dev *gf_dev)
{
	if (gf_dev->irq_enabled) {
		...
		enable_irq(gf_dev->irq);// Modified by zhangyuzhou for RESET_PIN test pin failed on 20170303
...
static void gf_disable_irq(struct gf_dev *gf_dev)
{
	if (gf_dev->irq_enabled) {
		...
		disable_irq(gf_dev->irq);// Modified by zhangyuzhou for RESET_PIN test pin failed on 20170303
	} else {
