...
//unsigned int FIRSTBOOT_SIZE = 0x8000;
//#define BOOTLOADER_CODESIZE  (332 * 1024)

unsigned int FIRSTBOOT_SIZE = 0x10000;
//unsigned int FIRSTBOOT_SIZE = 0x0000120c;
//unsigned int FIRSTBOOT_SIZE = 0x0004b2cc;
//unsigned int FIRSTBOOT_SIZE = 0x0004b2e0;
#define BOOTLOADER_CODESIZE  (364 * 1024)
...
int myread(FILE *binfile, char *buffer, int offset, int size) {
	if(filesize < offset) {
		return -1;
	}
...
static int getModuleTable(FILE *binfile, int m_table_idx, unsigned int off, module_header_t *m_table, size_t size) {
	...
	printf("LastModTableOffset:0x%08x\n",LastModTableOffset);
...
int show_module_table(FILE *binfile) {
	...
	for (int i = 0; i < MOD_END; ++i) {
		...
		{
			char *buffer = get_char_buffer(mod_info.lma_size);
			if(buffer == NULL) {
				return -1;
			}
			if(myread(binfile, (char *)buffer, (int)mod_info.lma, mod_info.lma_size) != 0) {
				return -1;
			}
			unsigned int crcval = crc32(buffer, mod_info.lma_size);
			printf("crcval:%08x\n", crcval);
			//printf("mod_info.crc32:%08x\n", mod_info.crc32);
			free(buffer);
		}
...
int process_header_info(FILE *binfile, bool b_fix, int fix_modindex, int fix_modsize) {
	...
	printf("\n bootloader size:=0X%X,BOOTLOADER_CODESIZE:=0X%X  \n",stMainHeadInfo.bootloader_size*BOOTLOADER_SIZE_UNIT, BOOTLOADER_CODESIZE);
	...
	if((stMainHeadInfo.flag[0] == 'S')
		...
		if(stMainHeadInfo.bootloader_size != 0)
		{
			module_info_t last_loader_info;
			last_loader_info.lma =0 ;
			last_loader_info.lma_size =0 ;
			...
			while(true) {
				...
				if((mod_info.lma > last_loader_info.lma)&&((unsigned int)mod_info.lma!=0xffffffff))
				{
					memcpy((unsigned char *)&last_loader_info, (unsigned char *)&mod_info, sizeof(module_info_t));
				}
			...
			if(((unsigned int)last_loader_info.lma +  last_loader_info.lma_size) % 4)
				loader_header_addr =(unsigned int)last_loader_info.lma +  last_loader_info.lma_size + 4 - ((unsigned int)last_loader_info.lma + last_loader_info.lma_size) % 4;
			else
				loader_header_addr =(unsigned int)last_loader_info.lma +  last_loader_info.lma_size;
			...
			printf("\n loader_header_info.head[0]:=%c  \n", loader_header_info.head[0]);
			printf("\n loader_header_info.head[1]:=%c  \n", loader_header_info.head[1]);
			printf("\n loader_header_info.head[2]:=%c  \n", loader_header_info.head[2]);
			printf("\n loader_header_info.head[3]:=%c  \n", loader_header_info.head[3]);
			printf("\n loader_header_info.head[4]:=%c  \n", loader_header_info.head[4]);
			printf("\n loader_header_info.head[5]:=%c  \n", loader_header_info.head[5]);
...
int main(int argc, char **argv) {
	...
	printf("infilename:%s modindex:%d modsize:%08x bi_fix:%s\n", infilename, modindex, modsize, b_fix ? "true" : "false");
	...
	if(show_module_table(binfile) != 0) {
		fclose(binfile);
		return -1;
	}

	if(process_header_info(binfile, b_fix, modindex, modsize) == 0) {
		printf("check OK!\n");
	} else {
		printf("GetRomCodeHeaderInfo error!\n");
	}
