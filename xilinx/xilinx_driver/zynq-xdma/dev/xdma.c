...
static int xdma_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	result = remap_pfn_range(vma, vma->vm_start,
			...
			page_to_pfn(virt_to_page(xdma_addr)),
...
void xdma_get_dev_info(char *name, struct xdma_dev *dev)
{
	int i;

	for (i = 0; i < MAX_DEVICES; i++) {
		if(strcmp(xdma_dev_info[i]->dma_name, name) != 0)
			break;
	}
...
static bool xdma_filter(struct dma_chan *chan, void *param)
{
	...
	if (strcmp(dma_chan_name(chan), pinfo->name) != 0)
		return false;
...
void xdma_probe(void)
{
	...
	const char *names[] = {"axi_sobel_vdma_1",0};
