...
AVL_uint32 uiData=0;
	...
	AVL_DVBSx_II2C_Read32(pAVLChip, pll_clkf_map_addr, &uiData);
	while(uiData!=pPLLConf->m_uiClkf )
		{
		printf("I2C FAIL..,uiData:%08x pPLLConf->m_uiClkf:%08x\n",uiData, pPLLConf->m_uiClkf);
	...
	r |= AVL_DVBSx_II2C_Write32(pAVLChip, reset_register_addr, 1);  //this is a reset, do not expect an ACK from the chip.
...
AVL_DVBSx_ErrorCode AVL_DVBSx_IBase_GetStatus( struct AVL_DVBSx_Chip * pAVLChip )
{
	...
	if( (AVL_DVBSx_EC_OK == r) )
	{
		if( (0 == uiTemp) || (uiReadValue != 0xA55A) )
		{
			...
			//printf(" AVL_DVBSx_IBase_GetStatus uiTemp:%d..%d\n",uiTemp,uiReadValue);
	}
	else
		printf(" AVL_DVBSx_IBase_GetStatus FAIL..%d\n",r);
	return(r);
}
