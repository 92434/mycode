...
#wget ftp://ftp.remotesensing.org/pub/libtiff/old/tiff-3.7.4.tar.gz
#fi
...
if [ ! -e zlib-1.2.5.tar.bz2 ]; then
#wget http://www.zlib.net/zlib-1.2.5.tar.bz2
wget http://down1.chinaunix.net/distfiles/zlib-1.2.5.tar.bz2
fi
tar -xjf zlib-1.2.5.tar.bz2
fi
cd zlib-1.2.5
...
if [ ! -d libpng-1.2.51 ]; then
if [ ! -e libpng-1.2.51.tar.bz2 ]; then
wget ftp://ftp.simplesystems.org/pub/png/src/libpng12/libpng-1.2.51.tar.bz2
#wget ftp://ftp.simplesystems.org/pub/png/src/libpng16/libpng-1.6.13.tar.gz
#wget ftp://ftp.simplesystems.org/pub/png/src/libpng-1.2.44.tar.bz2
#wget http://ncu.dl.sourceforge.net/project/libpng/libpng15/1.5.1/libpng-1.5.1.tar.bz2
fi
tar -xjf libpng-1.2.51.tar.bz2
fi
cd libpng-1.2.51
cat > $HOST.cache << "EOF"
...
#-------------------------------------------------------------------------------
# install DirectFB-1.6.3
#-------------------------------------------------------------------------------
if [ ! -e $PKG_CONFIG_PATH/directfb.pc ]; then
echo "installing DirectFB-1.6.3......"
if [ ! -d DirectFB-1.6.3 ]; then
if [ ! -e DirectFB-1.6.3.tar.gz ]; then
wget http://www.directfb.org/downloads/Core/DirectFB-1.6/DirectFB-1.6.3.tar.gz
fi
tar -xzf DirectFB-1.6.3.tar.gz
cat > .remove_davinci.patch <<EOF
--- DirectFB-1.6.3/gfxdrivers/Makefile.am	2014-09-17 16:52:42.651439043 +0800
+++ DirectFB-1.6.3/gfxdrivers/Makefile.am	2014-09-17 16:53:07.579438263 +0800
@@ -155,7 +155,6 @@
 	\$(ATI128_DIR)	\\
 	\$(CLE266_DIR)	\\
 	\$(CYBER5K_DIR)	\\
-	\$(DAVINCI_DIR)	\\
 	...
 	\$(GLES2_DIR)	\\
EOF
patch -p0 < .remove_davinci.patch
rm .remove_davinci.patch
fi
cd DirectFB-1.6.3
./autogen.sh
./configure --host=$HOST --build=$BUILD --prefix=$PREFIX && make && make install || exit
...
cd cairo-1.10.2
./configure --host=$HOST --build=$BUILD --prefix=$PREFIX --enable-directfb --disable-xlib --disable-win32 && make && make install || exit
cd ..
rm -rf cairo-1.10.2
fi
...
./configure --host=$HOST --build=$BUILD --prefix=$PREFIX --with-gdktarget=directfb --without-x && make && make install || exit
cd ..
rm -rf gtk+-2.24.3
fi
