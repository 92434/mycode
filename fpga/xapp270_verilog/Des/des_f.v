...
always @(sb1outa or sb1outb or xor_out_reg) begin
  if(xor_out_reg[1]) sbox_out[1:4] = sb1outb; 
  else sbox_out[1:4] = sb1outa;
end
always @(sb2outa or sb2outb or xor_out_reg) begin
  if(xor_out_reg[7]) sbox_out[5:8] = sb2outb; 
  else sbox_out[5:8] = sb2outa;
end
always @(sb3outa or sb3outb or xor_out_reg) begin
  if(xor_out_reg[13]) sbox_out[9:12] = sb3outb; 
  else sbox_out[9:12] = sb3outa;
end
always @(sb4outa or sb4outb or xor_out_reg) begin
  if(xor_out_reg[19]) sbox_out[13:16] = sb4outb; 
  else sbox_out[13:16] = sb4outa;
end
always @(sb5outa or sb5outb or xor_out_reg) begin
  if(xor_out_reg[25]) sbox_out[17:20] = sb5outb; 
  else sbox_out[17:20] = sb5outa;
end
always @(sb6outa or sb6outb or xor_out_reg) begin
  if(xor_out_reg[31]) sbox_out[21:24] = sb6outb; 
  else sbox_out[21:24] = sb6outa;
end
always @(sb7outa or sb7outb or xor_out_reg) begin
  if(xor_out_reg[37]) sbox_out[25:28] = sb7outb; 
  else sbox_out[25:28] = sb7outa;
end
always @(sb8outa or sb8outb or xor_out_reg) begin
  if(xor_out_reg[43]) sbox_out[29:32] = sb8outb; 
  else sbox_out[29:32] = sb8outa;
end
...
//assign f_out_l = f_in_r_reg;
//assign f_out_r = pbox_out ^ f_in_l_reg;
assign f_out_l = f_in_r;
assign f_out_r = pbox_out ^ f_in_l;
